...
//geting year
	...
	public HashMap<String, Object> getAllCourseRelatedByIdYear(int id) {
		...
		if (courseById != null) {
			...
			coursesDto.setYear1Dto(courseById.getYear1().stream().map(year1 -> this.mapper.map(year1, Year1Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear2Dto(courseById.getYear2().stream().map(year2 -> this.mapper.map(year2, Year2Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear3Dto(courseById.getYear3().stream().map(year3 -> this.mapper.map(year3, Year3Dto.class))
					.collect(Collectors.toList())); 
			
			
			coursesDto.setYear4Dto(courseById.getYear4().stream().map(year4 -> this.mapper.map(year4, Year4Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear5Dto(courseById.getYear5().stream().map(year5 -> this.mapper.map(year5, Year5Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear6Dto(courseById.getYear6().stream().map(year6 -> this.mapper.map(year6, Year6Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear7Dto(courseById.getYear7().stream().map(year7 -> this.mapper.map(year7, Year7Dto.class))
					.collect(Collectors.toList()));
			
			coursesDto.setYear8Dto(courseById.getYear8().stream().map(year8 -> this.mapper.map(year8, Year8Dto.class))
					.collect(Collectors.toList()));
			...
			if (coursesDto.getYear1Dto() .size() > 0) {
				data.put("YearOne", coursesDto.getYear1Dto());
			}
			
			if (coursesDto.getYear2Dto() .size() > 0 ) {
				data.put("YearTwo", coursesDto.getYear2Dto());
			}
			
			if (coursesDto.getYear3Dto() .size() > 0) {
				data.put("YearThree", coursesDto.getYear3Dto());
				
			}
			
			if (coursesDto.getYear4Dto() .size() > 0) {
				data.put("YearFour", coursesDto.getYear4Dto());
			}
			
			if (coursesDto.getYear5Dto() .size() > 0) {
				data.put("YearFive", coursesDto.getYear5Dto());
			}
			
			if (coursesDto.getYear6Dto() .size() > 0) {
				data.put("YearSix", coursesDto.getYear6Dto());
			}
			
			if (coursesDto.getYear7Dto() .size() > 0) {
				data.put("YearSeven", coursesDto.getYear7Dto());
			}
			
			if (coursesDto.getYear8Dto() .size() > 0) {
				data.put("YearEight", coursesDto.getYear8Dto());
			}
...
//		return this.mapper.map(courseById, CoursesDto.class);
		HashMap<String, Object> data = new HashMap<>();
		data.put("YearOne", "hello");
		return data;
	}
